...
<blockquote>例: Sales Confirmation → TRES BUMI SDN BHD、Invoice → RAYA KHAS SDN BHD</blockquote>
  ...
  },
  {
    ...
    content: `
<h1>カテゴリ（車 / バイク）</h1>
<p>案件は <strong>🚗 自動車</strong> と <strong>🏍 オートバイ</strong> の2カテゴリに分類されます。各画面で切り替えて表示できます。</p>

<h2>カテゴリの設定</h2>
<ul>
  <li><strong>案件編集</strong> → 案件情報タブの先頭にカテゴリラジオボタン</li>
  <li><strong>車両モデル管理</strong> → モデル登録時にカテゴリを選択</li>
  <li>モデルを案件に「適用」すると、モデルのカテゴリが案件にも自動反映</li>
</ul>

<h2>各画面でのカテゴリ切り替え</h2>

<h3>📋 案件一覧</h3>
<p>ツールバー下に「すべて / 🚗 自動車 / 🏍 オートバイ」のタブ。クリックでフィルタが切り替わります。</p>

<h3>🚗 車両モデル管理</h3>
<p>ツールバーに同じカテゴリタブ。カテゴリ列がテーブルに表示されます。</p>

<h3>📊 ダッシュボード</h3>
<p>ヘッダー直下にカテゴリタブ。すべてのKPI・チャート・カンバンがフィルタに連動します。<br>
例: 「🏍 オートバイ」に切り替えると、バイク案件だけの売上・粗利・進捗を確認できます。</p>

<h3>🚢 Buyerポータル（プレビュー）</h3>
<p>1つのBuyerに車とバイク両方がある場合、自動的に <code>🚗 Cars</code> と <code>🏍 Motorcycles</code> のセクションに分割表示されます。</p>

<h2>カテゴリと書類</h2>
<p>書類のレイアウトは現在カテゴリで変わりません（同じテンプレート）。バイク用に別レイアウトが必要な場合は別途対応します。<br>
HSコードは案件側で個別設定できますが、参考として:</p>
<ul>
  <li>🚗 自動車: <code>8703.xx</code></li>
  <li>🏍 オートバイ: <code>8711.xx</code></li>
</ul>

<h2>既存案件の自動カテゴリ判定</h2>
<p>新カラム追加時は全案件が <strong>🚗 自動車</strong> として扱われます。バイクの案件は手動で編集してカテゴリ変更してください。</p>
    `,
  },
  {
    id: 'models',
...
<blockquote>モデル定義を後から編集しても、既に発行済みの書類の内容は変わりません（案件に値がコピー保存されるため）。</blockquote>
  ...
  {
    id: 'dereg-forms-reference',
    title: '様式リファレンス・書類アーカイブ・印刷版生成',
    content: `
<h1>様式リファレンス・書類アーカイブ・印刷版生成</h1>
<p>輸出抹消手続きで使う各様式について、サンプル閲覧・記入例・印刷版生成・提出後のスキャン保管までを一貫してサポートします。</p>

<h2>📑 様式リファレンス（ガイドページ右サイドバー）</h2>
<p>「輸出抹消手続き」タブの右サイドバーに、以下7種の様式が一覧表示されます:</p>
<ul>
  <li>💴 手数料納付書</li>
  <li>📄 第1号様式 申請書</li>
  <li>🚢 第3号様式の2 申請書/届出書</li>
  <li>📑 登録識別情報等通知書</li>
  <li>📝 譲渡証明書（第21号様式）</li>
  <li>🔐 印鑑証明書</li>
  <li>♻️ リサイクル券（A券）</li>
</ul>
<p>各様式をクリック → モーダルでサンプルPDFが表示され、以下も併せて確認できます:</p>
<ul>
  <li>主要記入項目（業務種別コード・期待値付き）</li>
  <li>案件データから自動表示される実値（案件選択時）</li>
  <li>📑 サンプルPDF画像（実物の見本）</li>
</ul>

<h2>🔢 業務種別コード・抹消区分コード 参照表</h2>
<p>右サイドバーに2種のコード参照表を常設:</p>
<ul>
  <li><strong>業務種別コード</strong>: 1011 移転登録 / 1331 輸出抹消仮登録 / 1310 永久抹消登録 / 1320 一時抹消登録</li>
  <li><strong>抹消区分コード</strong>: 1 解体 / 2 一時使用中止 / <strong>3 輸出</strong> / 4 減失 / 5 用途廃止 / 6 再輸入 / 7 再輸入見込</li>
</ul>

<h2>🖨 印刷版の生成（Phase 4）</h2>
<p>3様式について、案件データを差し込んだ印刷可能版を新規ウィンドウで生成できます:</p>
<ul>
  <li>💴 手数料納付書</li>
  <li>📄 第1号様式</li>
  <li>🚢 第3号様式の2</li>
...
<div class="help-step">
  <div class="help-step__num">1</div>
  <div class="help-step__body"><p>「輸出抹消手続き」タブで案件を選択</p></div>
</div>
<div class="help-step">
  <div class="help-step__num">2</div>
  <div class="help-step__body"><p>右サイドバーの様式をクリック → モーダルで「🖨 案件データから印刷可能版を生成」</p></div>
</div>
<div class="help-step">
  <div class="help-step__num">3</div>
  <div class="help-step__body"><p>新ウィンドウが開き、印刷ダイアログ表示 → PDF保存または用紙印刷</p></div>
</div>

<blockquote class="warn">⚠️ システム生成版は<strong>下書きレベル</strong>です。実際の提出は国土交通省の公式様式（PDF）を使用してください。生成版は記入内容の確認・社内用ドラフトとして活用できます。</blockquote>

<h2>🗄 提出書類アーカイブ（Phase 3）</h2>
<p>案件編集画面に新タブ「<strong>提出書類アーカイブ</strong>」を追加。提出書類のスキャン/写真を案件ごとに保管できます。</p>
<ul>
  <li>7種類の書類タイプ別フォルダ</li>
  <li>ドラッグ&ドロップでアップロード（PDF / 画像 / 5MB以下）</li>
  <li>画像は自動リサイズ（長辺2000px）</li>
  <li>誰がいつアップロードしたか自動記録</li>
  <li>クリックで新タブ表示（拡大確認）</li>
</ul>

<h2>運用フローの想定例</h2>
<ol>
  <li><strong>事前確認</strong>: ガイドページで様式リファレンスを確認、業務種別コードを暗記</li>
  <li><strong>印刷版生成</strong>: 案件を選択 → 印刷版を生成 → 社内チェック</li>
  <li><strong>本記入</strong>: 国交省公式様式に手書きまたはOSSで記入・提出</li>
  <li><strong>提出後</strong>: 提出書類アーカイブで全書類のスキャンを保管 → 後日の参照やトラブル対応に</li>
</ol>
    `,
  },
  {
    ...
    content: `
<h1>輸出抹消手続きガイド</h1>
<p>ヘッダー上部の「輸出抹消手続き」タブから、<strong>手続き方法とチェックリストを一覧で見られる</strong>ガイドページが開けます。</p>

<h2>ページ構成</h2>
<ul>
  <li><strong>📖 概要</strong>: 全体フローを5ステップでざっくり把握</li>
  <li><strong>左カラム: 手順詳細</strong>: 各ステップを番号付きカードで表示。書類ごとに詳細解説と「なぜ必要か」の説明付き</li>
  <li><strong>右カラム: リファレンス</strong>
    <ul>
      <li>📑 提出する様式（第3号様式の2 / 第1号様式 / 第3号様式 / 印鑑証明書 等）</li>
      <li>✏️ 記載項目チェック（業務種別コード等の期待値）</li>
      <li>🔗 参考リンク（国交省公式・様式ダウンロード）</li>
    </ul>
  </li>
</ul>
...
<h3>① 一般ガイドモード（案件未選択）</h3>
<p>純粋な手順説明書として閲覧。チェックボックスは無効。新人教育や手続き内容の確認用。</p>

<h3>② 案件進捗モード（案件選択時）</h3>
<p>ドロップダウンで案件を選択すると、その案件のチェックリストとして機能。チェックを入れると自動保存され、案件編集側の「輸出抹消手続き」タブと連動。</p>
<ul>
  <li>「📌 名義変更が必要な案件」チェックで移転登録グループが活性化</li>
  <li>右上に進捗率（X/Y N%）が常時表示</li>
  <li>「自動車登録番号」「車台番号」が案件データから自動表示される</li>
</ul>

<h2>案件編集の「輸出抹消手続き」タブとの違い</h2>
<table>
  <tr><th>項目</th><th>案件編集タブ</th><th>ガイドページ</th></tr>
  <tr><td>場所</td><td>案件編集の9タブ目</td><td>トップレベルナビ</td></tr>
  <tr><td>用途</td><td>案件作業中に素早くチェック</td><td>手順を確認しながら作業</td></tr>
  <tr><td>説明文の量</td><td>最小限</td><td>各項目に詳しい解説</td></tr>
  <tr><td>レイアウト</td><td>縦長フラット</td><td>左右2カラム + リファレンス</td></tr>
  <tr><td>案件切替</td><td>案件編集を切替</td><td>ドロップダウンで切替</td></tr>
</table>
<p>進捗データは <strong>両者で同じテーブル</strong>に保存されるので、どちらで操作しても連動します。</p>

<blockquote>💡 普段は「ガイドページ」をブックマーク的に開きっぱなしにし、複数案件の進捗を切り替えながら作業すると効率的です。</blockquote>
    `,
  },
  {
    id: 'dereg-checklist',
    title: '輸出抹消手続きチェックリスト（案件内）',
    content: `
